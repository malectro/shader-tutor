...
retain the information in the lectures. I similarly was told by multiple music instructors
...
workflow, I occasionally found myself doing something the "hard way" just so I wouldn't
...
underlying parsable data structure.


## Measuring Success

Success of the Shader Tutor couldn't simply come down to usage numbers. If it's just fun
and not useful then I've made a viable business but failed as an educator. That said, it
...
don't provide any assistance, but in the age of LLMs, this could easily be gamed. A
combination of usage numbers and survey data would be a good heuristic at least.

## Scalability

Because the Shader Tutor can run as a lambda or edge function, it's more scalable than an
app with a server and a database. Obviously I'd need to work out how to support millions of
users making requests to the Claude API – either by letting them supply their own tokens
or entering into an enterprise contract and charging users subscriptions (which would mean
some sort of account management).
...
- folded in the "explain this piece of code" feature. This was very useful for GLSL because
